...
for i in `cat /compile/doc/stable-r2/misc.rk2/options/additional-options-*-yes.txt`; do
  echo $i
  ./scripts/config -e $i
done

for i in `cat /compile/doc/stable-r2/misc.rk2/options/additional-options-*-mod.txt`; do
  echo $i
  ./scripts/config -m $i
done
